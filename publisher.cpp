...
bool Publisher::publish(const ProjectData &project,
  ...
  if (success) {
    ...
    // Let's assume specific methods return false on error and emit
    // finished(false, reason) internally? No, better design: return
    // success/fail and emit finished HERE.
  }
  return success;
}

bool Publisher::publishLinux(const ProjectData &project,
                             const PublishConfig &config) {
  ...
  QString homeRuntime = QDir::homePath() + "/.bennugd2/runtime/linux-gnu";
  if (QDir(homeRuntime).exists()) {
    ...
    for (int i = 0; i < 4; i++) {
      ...
      candidate = searchDir.absoluteFilePath("runtime/linux-gnu");
      if (QDir(candidate).exists()) {
        runtimeDir = candidate;
        break;
      }
  ...
  if (!QFile::exists(sourceDcbPath)) {
    emit finished(
        false,
        "No se encontró el archivo compilado (.dcb).\n"
  ...
  if (bgdiPath.isEmpty()) {
    ...
    if (bgdiPath.isEmpty()) {
      ...
      if (!QFile::exists(bgdiPath)) {
        emit finished(false,
                      "No se encontró el intérprete bgdi.\n"
  ...
  QFile(distDir + "/bgdi")
      .setPermissions(QFile::ExeUser | QFile::ExeGroup | QFile::ExeOther |
  ...
  if (!runtimeDir.isEmpty() && QDir(runtimeDir).exists()) {
    ...
    QStringList filters;
    filters << "*.so*";
    QFileInfoList runtimeLibs =
        runtimeLibDir.entryInfoList(filters, QDir::Files);
  ...
  QString wrapperSrc =
      QCoreApplication::applicationDirPath() + "/" + wrapperName;
  if (!QFile::exists(wrapperSrc))
    wrapperSrc = QDir::currentPath() + "/" + wrapperName;
  ...
  if (QFile::exists(wrapperSrc)) {
    ...
    if (script.open(QIODevice::WriteOnly | QIODevice::Text)) {
      ...
      script.close();
      script.setPermissions(QFile::ExeUser | QFile::ExeGroup | QFile::ExeOther |
  ...
  if (config.generateLinuxStandalone) {
    ...
    if (QFile::exists(stubPath)) {
      QString standalonePath =
          config.outputPath + "/" + baseName +
          ".AppImage"; // Using .AppImage extension often implies single-file to
                       // users, or just .bin
      standalonePath =
          config.outputPath + "/" + baseName + "_linux.bin"; // Better name
      ...
      for (const auto &info : libFiles) {
        ...
        if (fLib.open(QIODevice::ReadOnly)) {
          // We put libs in root or lib/ subfolder. Stub sets LD_LIBRARY_PATH to
          // . and ./lib Let's keep them in root for simplicity or lib/
          filesToEmbed.append({info.absoluteFilePath(),
                               "lib/" + info.fileName(), fLib.readAll()});
          fLib.close();
        }
      }

      // 4. Add Assets (Recursive)
      QString projectDir = QFileInfo(project.path).isDir()
                               ? project.path
                               : QFileInfo(project.path).absolutePath();
      QDirIterator it(projectDir, QDir::Files | QDir::NoDotAndDotDot,
      ...
      while (it.hasNext()) {
        ...
        if (relPath.startsWith("build") || relPath.startsWith("dist") ||
            relPath.startsWith("."))
          continue;
        if (info.suffix() == "dcb" || info.suffix() == "prg")
          continue;
        if (info.fileName() == "bgdi" ||
            info.fileName().startsWith("loader_stub"))
          continue;
      ...
      if (!project.iconPath.isEmpty() && QFile::exists(project.iconPath)) {
        ...
        QString desktopContent =
            QString(
                ...
                .arg(project.name);
        filesToEmbed.append(
            {"", baseName + ".desktop", desktopContent.toUtf8()});
      }
      ...
      if (stubFile.open(QIODevice::ReadOnly) &&
        ...
        // Make executable
        outFile.setPermissions(QFile::ExeUser | QFile::ExeGroup |
                               QFile::ExeOther | QFile::ReadOwner |
  ...
  if (config.generateAppImage) {
    ...
    if (cleanAppDir.exists())
      cleanAppDir.removeRecursively();
    ...
    QFile(appDir + "/usr/bin/" + baseName)
        .setPermissions(QFile::ExeUser | QFile::ExeGroup | QFile::ExeOther |
                        QFile::ReadOwner | QFile::ReadGroup | QFile::ReadOther);

    // Copy DCB to AppDir/usr/bin
    QFile::copy(distDir + "/" + baseName + ".dcb",
                appDir + "/usr/bin/" + baseName + ".dcb");

    // 2. Copy Libs
    copyDir(libDir.absolutePath(), appDir + "/usr/lib");

    // 3. Copy Assets relative to binary (classic method) or in share (standard)
    // Bennu usually expects assets near binary or in current dir. AppRun
    // handles current dir. We will put assets in usr/bin so they are next to
    // executable, easiest for Bennu.
    copyDir(assetsDir.absolutePath(), appDir + "/usr/bin/assets");

    // Remove tmp dist dir if we are only making appimage? No, keep it as
    // "unpacked" version maybe? User asked for .tar.gz AND AppImage possibly.
    ...
    if (appRun.open(QIODevice::WriteOnly | QIODevice::Text)) {
      ...
      appRun.close();
      appRun.setPermissions(QFile::ExeUser | QFile::ExeGroup | QFile::ExeOther |
                            QFile::ReadOwner | QFile::ReadGroup |
                            QFile::ReadOther);
    }
    ...
    if (!config.appImageToolPath.isEmpty() &&
        QFile::exists(config.appImageToolPath)) {
      toolExe = config.appImageToolPath;
      // Ensure executable
      QFile::setPermissions(toolExe, QFile::ExeUser | QFile::ExeGroup |
                                         QFile::ExeOther | QFile::ReadOwner);
    } else {
      ...
      if (QStandardPaths::findExecutable("appimagetool").isEmpty()) {
        emit finished(true, "AppDir creado en " + appDir +
                                ".\nInstala 'appimagetool' o configúralo para "
    ...
    if (toolInfo.size() < 1024 * 1024) { // Less than 1MB is suspicious
      emit finished(
          false,
          "Error: El archivo appimagetool parece corrupto o incompleto (" +
              QString::number(toolInfo.size()) +
              " bytes).\n"
              "Por favor borra " +
              toolExe + " y vuelve a descargarlo desde el diálogo.");
      return false;
    }
    ...
    if (!checkRun.waitForFinished() || checkRun.exitCode() != 0) {
      ...
      qDebug()
          << "Standard execution failed. Trying APPIMAGE_EXTRACT_AND_RUN=1...";
      ...
      if (!checkRun2.waitForFinished() || checkRun2.exitCode() != 0) {
        ...
        emit finished(false,
                      "No se puede ejecutar appimagetool incluso sin FUSE.\n"
                      "Código de salida: " +
                          QString::number(checkRun2.exitCode()) +
                          "\n"
                          "Salida (stdout): " +
                          out +
                          "\n"
    ...
    // Run with --no-appstream to avoid validation errors and --verbose for
    // debug Note: appImageTool environment might have been updated with
    // EXTRACT_AND_RUN above

    QString finalAppImagePath =
        config.outputPath + "/" + baseName + ".AppImage";
    if (QFile::exists(finalAppImagePath))
      QFile::remove(finalAppImagePath);

    appImageTool.start(toolExe, QStringList()
                                    << "--no-appstream" << "--verbose"
                                    << "AppDir" << baseName + ".AppImage");
    if (appImageTool.waitForFinished() && appImageTool.exitCode() == 0) {
      ...
      if (!error.isEmpty())
        ...
        msg += "Código de salida: " + QString::number(appImageTool.exitCode());

      emit finished(true,
                    msg); // Emit true so dialog stays open but warns? Or false?
      // If we return 'true' here, it says "Publication Successful" then shows
      // error msg. Better emit 'false' but since the tar.gz MIGHT have
      // succeeded... Let's emit finished with false to show the error clearly.
      // Actually, if we are here, we might have already done tar.gz?
      // No, tar.gz is after this block in my code logic!

      // So if AppImage fails, we fail completely (or we should continue to
      // tar.gz?) Let's fail for now to show the error.
      emit finished(false, msg);
      return false;
    }
  }

  // Always do tar.gz if requested (default logic was else, changed to
  // sequential if checkboxes allow) Actually the dialog has checkboxes.
  // Publisher::publishLinux only receives `config`. We should check config.
  ...
  // Let's just create tar.gz as primary artifact always for now unless I
  // rewrite structure. I'll leave the tar logic as is (it runs if AppImage is
  // FALSE in my previous code, wait). My previous code: "if
  // (config.generateAppImage) { ... } else { emit progress... tar ... }" That
  // prevents both.
...
bool Publisher::publishAndroid(const ProjectData &project,
  ...
  QString homeRuntime = QDir::homePath() + "/.bennugd2/runtime/android";
  QString runtimeAndroid;

  if (QDir(homeRuntime).exists()) {
    runtimeAndroid = homeRuntime;
  } else {
    runtimeAndroid = appPath + "/runtime/android";
  }
  ...
  if (QFile::exists(templateDir + "/gradlew")) {
    ...
    QFile::copy(templateDir + "/gradle/wrapper/gradle-wrapper.jar",
                targetDir + "/gradle/wrapper/gradle-wrapper.jar");
    QFile::copy(templateDir + "/gradle/wrapper/gradle-wrapper.properties",
                targetDir + "/gradle/wrapper/gradle-wrapper.properties");
  } else {
    emit progress(5, "ADVERTENCIA: No se encontró plantilla Gradle (gradlew) "
                     "en runtime/android/template.");
  }
  ...
  if (sdkDir.isEmpty())
    sdkDir = qgetenv("ANDROID_SDK_ROOT");
  if (sdkDir.isEmpty())
    sdkDir = QStandardPaths::writableLocation(QStandardPaths::HomeLocation) +
             "/Android/Sdk";
  ...
  // 7. gradlew wrapper (We should download current wrapper or write a basic
  // script) To be truly professional, we should carry the gradle-wrapper.jar
  // and properties. For now, let's write a minimal gradlew script that invokes
  // 'gradle' is safer if we don't bundle the jar.
  ...
  QString activityName =
      "Activity_" + project.name.simplified().replace(" ", "_");
  ...
  if (java.open(QIODevice::WriteOnly | QIODevice::Text)) {
    ...
    // Base libs
    QStringList baseLibs = {
        "ogg",        "vorbis",   "vorbisfile", "theoradec",
        "theoraenc",  "theora",   "SDL2",       "SDL2_image",
        "SDL2_mixer", "SDL2_gpu", "bgdrtm",     "bggfx",
        "bginput",    "bgload",   "bgsound",    "sdlhandler"};

    for (const QString &lib : baseLibs) {
      out << "            \"" << lib << "\",\n";
    }

    // Scan for ALL modules (libmod_*.so) in runtime to ensure dependencies are
    // met
    ...
    out << "        android.util.Log.d(\"BennuDebug\", \"Starting Asset "
           "Extraction...\");\n";
    out << "        recursiveCopy(\"\");\n";
    out << "        \n";
    out << "        // DEBUG: Check specific file\n";
    out << "        File testFile = new File(getFilesDir(), "
           "\"assets/fpg/level1.fpg\");\n";
    out << "        android.util.Log.d(\"BennuDebug\", \"CHECK FILE exists: \" "
    ...
    out << "             for(String f : rootFiles) "
           "android.util.Log.d(\"BennuDebug\", \"ROOT FILE: \" + f);\n";
    out << "        }\n\n";
    out << "        // 2. Change Working Directory via Reflection (Robust "
           "Loop)\n";
    out << "        android.util.Log.d(\"BennuDebug\", \"INTENTANDO "
           "CHDIR...\");\n";
    out << "        try {\n";
    out << "            Class<?> osClass = "
    ...
    out << "                    android.util.Log.d(\"BennuDebug\", \"Found "
           "chdir method, invoking...\");\n";
    out << "                    m.invoke(null, "
           "getFilesDir().getAbsolutePath());\n";
    out << "                    android.util.Log.d(\"BennuDebug\", \"CHDIR "
           "SUCCESS\");\n";
    out << "                    break;\n";
    out << "                }\n";
    out << "            }\n";
    out << "        } catch (Throwable e) {\n";
    out << "            android.util.Log.e(\"BennuDebug\", \"CHDIR FAILED\", "
           "e);\n";
    ...
    out << "        return new String[] { new java.io.File(getFilesDir(), "
           "\"game.dcb\").getAbsolutePath() };\n";
    out << "    }\n";
    out << "    @Override\n";
    ...
    for (const QString &abi : abis) {
      ...
      if (QDir(srcLibDir).exists()) {
        ...
        for (const QFileInfo &fi : dir.entryInfoList(filters, QDir::Files)) {
          ...
          if (QFile::copy(fi.absoluteFilePath(),
                          destLibDir + "/" + fi.fileName())) {
            totalCopied++;
          }
        }
      }
  ...
  if (!usedRuntime) {
    emit progress(
        63, "Buscando librerías en árbol de fuentes (modo desarrollo)...");
    ...
    if (QFile::exists(adsJava))
      QFile::copy(adsJava, sdlPackagePath + "/AdsModule.java");
    if (QFile::exists(iapJava))
      QFile::copy(iapJava, sdlPackagePath + "/IAPModule.java");
    ...
    for (int i = 0; i < 8; i++) {
      ...
      if (searchDir.isRoot() || !searchDir.cdUp())
        break;
    }
    ...
    if (projectRoot.isEmpty()) {
      ...
      for (auto it = toolchainToAbi.constBegin();
           it != toolchainToAbi.constEnd(); ++it) {
        ...
        if (QDir(buildBinDir).exists()) {
          ...
          QStringList filters;
          filters << "*.so";
          for (const QFileInfo &soFile :
               binDir.entryInfoList(filters, QDir::Files)) {
        ...
        QString vendorLibDir =
            projectRoot + "/vendor/android/" + toolchain + "/" + abi + "/lib";
        if (QDir(vendorLibDir).exists()) {
          QDir libDir(vendorLibDir);
          QStringList filters;
          filters << "*.so" << "*.so.*";
          for (const QFileInfo &soFile :
               libDir.entryInfoList(filters, QDir::Files)) {
            QString destPath = abiLibDir + "/" + soFile.fileName();
            QFile::remove(destPath);
        ...
        // 3. Copy SDL_gpu
        QString sdlGpuLib = projectRoot + "/vendor/sdl-gpu/build/" + toolchain +
                            "/SDL_gpu/lib/libSDL2_gpu.so";
      ...
      if (!hasBennuLibs) {
        emit progress(70, "ADVERTENCIA: Faltan librerías de BennuGD en entorno "
                          "de desarrollo.");
      }
    }
  }
  ...
  if (QFile::exists(runtimeManifest)) {
    ...
    QFile::copy(runtimeManifest,
                targetDir + "/app/src/main/AndroidManifest.xml");
  }
  ...
  if (manifest.open(QIODevice::ReadOnly | QIODevice::Text)) {
    ...
    manifest.close();
    content.replace("package=\"org.libsdl.app\"",
                    "package=\"" + config.packageName + "\"");
    content.replace("android:name=\"SDLActivity\"",
  ...
  if (!QFile::exists(sourceDcbPath)) {
    emit finished(false, "No se encontró el archivo compilado (.dcb).\n"
                         "Por favor, compila el proyecto en el editor antes de "
                         "generar para Android.");
    return false;
  }
  ...
  QDirIterator it(sourceRoot.absolutePath(), QDir::Files | QDir::NoDotAndDotDot,
                  QDirIterator::Subdirectories);
  ...
  while (it.hasNext()) {
    ...
    if (relPath.startsWith("android/") || relPath.startsWith("build/") ||
        relPath.startsWith("ios/") || relPath.startsWith(".git") ||
        relPath.contains("/.") || relPath.endsWith(".prg") ||
        relPath.endsWith(".dcb") || relPath.endsWith(".o") ||
        relPath.endsWith(".a") || relPath.endsWith(".user")) {
      continue;
    }
  ...
  if (config.generateAPK || config.installOnDevice) {
    QString actionName = config.installOnDevice
                             ? "Instalando en dispositivo (esto tarda)..."
                             : "Generando APK...";
    ...
    QFile(targetDir + "/gradlew")
        .setPermissions(QFile::ExeUser | QFile::ExeOwner | QFile::ReadOwner |
                        QFile::WriteOwner);

    QString task = config.installOnDevice ? "installDebug" : "assembleDebug";

    // Use environment ANDROID_HOME if available, otherwise rely on
    // local.properties generated by us or gradle
    if (!config.ndkPath.isEmpty()) {
    ...
    if (javaHome.isEmpty() || !QDir(javaHome).exists()) {
      ...
      for (const QString &path : candidates) {
        if (QDir(path).exists() && (QFile::exists(path + "/bin/java") ||
                                    QFile::exists(path + "/bin/java.exe"))) {
          javaHome = path;
    ...
    if (!javaHome.isEmpty()) {
      ...
      javaBin.setPermissions(javaBin.permissions() | QFile::ExeUser |
                             QFile::ExeOwner);
    ...
    // Execute Gradle Wrapper directly via Java to bypass script detection
    // issues
    ...
    if (gradleProc.waitForFinished(-1) && gradleProc.exitCode() == 0) {
      if (config.installOnDevice) {
        ...
        if (sdk.isEmpty())
          sdk = QStandardPaths::writableLocation(QStandardPaths::HomeLocation) +
                "/Android/Sdk";
        if (QFile::exists(sdk + "/platform-tools/adb"))
          adbExe = sdk + "/platform-tools/adb";

        // Launch app using adb monkey trick (launches main activity of package)
        QProcess adb;
        adb.start(adbExe, QStringList()
                              << "shell" << "monkey" << "-p"
                              << config.packageName << "-c"
...
// Fallback if x-terminal-emulator not found? user usually has one mapped.
      ...
      if (err.isEmpty())
        err = gradleProc.readAllStandardOutput(); // Sometimes gradle prints
                                                  // error to stdout

      QString advice =
          config.installOnDevice
              ? "Verifica que tienes dispositivo conectado y depuración USB."
              : "Verifica configuración de SDK/Java.";
      if (javaHome.isEmpty())
...
bool Publisher::publishWindows(const ProjectData &project,
                               const PublishConfig &config) {
  ...
  // Priority: User Home (check win64 and windows aliases)
  QStringList homeCandidates;
  homeCandidates << QDir::homePath() + "/.bennugd2/runtime/win64"
                 << QDir::homePath() + "/.bennugd2/runtime/windows";

  for (const QString &homeCand : homeCandidates) {
    if (QDir(homeCand).exists()) {
      runtimeDir = homeCand;
      break;
    }
  }
  ...
  if (runtimeDir.isEmpty()) {
    // Fallback: Bundled
    for (int i = 0; i < 4; i++) {
      if (searchDir.exists("runtime/win64")) {
        runtimeDir = searchDir.absoluteFilePath("runtime/win64");
        break;
      } else if (searchDir.exists("runtime/windows")) {
        runtimeDir = searchDir.absoluteFilePath("runtime/windows");
        break;
      }
      searchDir.cdUp();
    }
  ...
  if (runtimeDir.isEmpty()) {
    emit finished(
        false,
        "No se encontró el directorio de runtime de Windows.\n"
        "Se buscó en ~/.bennugd2/runtime/win64 (o /windows) y en el directorio "
        "de la aplicación.\n"
        "Asegúrate de tener los binarios de Windows instalados.");
    return false;
  }
  ...
  QString bgdiExePath;
  // Check root and bin/
  if (QFile::exists(runtimeDir + "/bgdi.exe")) {
    bgdiExePath = runtimeDir + "/bgdi.exe";
  } else if (QFile::exists(runtimeDir + "/bin/bgdi.exe")) {
    bgdiExePath = runtimeDir + "/bin/bgdi.exe";
  }

  if (bgdiExePath.isEmpty()) {
    emit finished(false,
                  "No se encontró bgdi.exe en " + runtimeDir +
                      "\n"
                      "Se buscó en la raíz y en el subdirectorio bin/.\n"
                      "Asegúrate de tener el ejecutable de Windows instalado.");
    return false;
  }
  ...
  if (launcherVbs.open(QIODevice::WriteOnly | QIODevice::Text)) {
    ...
    out << "WshShell.CurrentDirectory = "
           "CreateObject(\"Scripting.FileSystemObject\").GetParentFolderName("
           "WScript.ScriptFullName)\r\n";
    out << "WshShell.Run \"bgdi.exe \"\"" << baseName
        << ".dcb\"\"\", 0, False\r\n";
    launcherVbs.close();
  ...
  if (readme.open(QIODevice::WriteOnly | QIODevice::Text)) {
    ...
    out << "   (o '" << baseName
        << ".vbs' para ejecutar sin ventana de consola)\r\n\r\n";
  ...
  if (config.generateStandalone) {
    ...
    if (QFile::exists(stubPath)) {
      ...
      struct FileToEmbed {
        QString sourcePath;
        QString relativePath; // Path relative to execution root (e.g.
                              // "bgdi.exe", "assets/player.png")
        QByteArray data;
      };
      QVector<FileToEmbed> filesToEmbed;
      ...
      if (dcbFile.open(QIODevice::ReadOnly)) {
        ...
        emit finished(false,
                      "No se pudo leer el archivo .dcb para empaquetado.");
        return false;
      }
      ...
      QStringList dllFilters;
      dllFilters << "*.dll";
      QFileInfoList dllFiles =
          runtimeDirObj.entryInfoList(dllFilters, QDir::Files);
      for (const QFileInfo &dllInfo : dllFiles) {
        QFile f(dllInfo.absoluteFilePath());
        if (f.open(QIODevice::ReadOnly)) {
          filesToEmbed.append(
              {dllInfo.absoluteFilePath(), dllInfo.fileName(), f.readAll()});
          f.close();
        }
      }
      ...
      // - Hidden files
      // - Source code files (.prg, .c, .h, .cpp) - Optional: user might want to
      // hide source
      ...
      while (it.hasNext()) {
        ...
        if (relPath.startsWith("build") || relPath.startsWith("dist") ||
          ...
          continue;
        if (info.suffix() == "dcb" || info.suffix() == "prg")
          continue; // Skip redundant binaries/source
        if (info.fileName() == "bgdi.exe" ||
            info.fileName() == "loader_stub.exe")
          continue;
      ...
      if (stubFile.open(QIODevice::ReadOnly) &&
        ...
        for (const auto &file : filesToEmbed) {
          ...
          // Convert relative path to standard backslashes for Windows, or keep
          // forward slash (stub handles normalization)
          QByteArray pathBytes = file.relativePath.toUtf8();
          strncpy(entry.path, pathBytes.constData(), 255);
          entry.size = (uint32_t)file.data.size();
        ...
        qDebug() << "Created V3 standalone executable with" << toc.size()
                 << "total files.";
  ...
  if (config.generateSfx) {
    ...
    if (!sevenZExe.isEmpty()) {
      ...
      archiveArgs << "a" << "-t7z" << "-mx=9" << baseName + "_win64.7z"
                  << baseName + "_win64";

      sevenZipProcess.start(sevenZExe, archiveArgs);
      if (sevenZipProcess.waitForFinished(60000) &&
          sevenZipProcess.exitCode() == 0) {
        qDebug() << "Created 7z archive";

        // Now try to create SFX
        // Look for 7z SFX module
        QString sfxModule;
        QStringList sfxPaths = {"/usr/lib/p7zip/7z.sfx",
        ...
        if (!sfxModule.isEmpty()) {
          ...
          QString sfxExePath =
              config.outputPath + "/" + baseName + "_win64.exe";
  ...
  if (config.generateZip) {
    ...
    if (zipProcess.waitForFinished(30000)) {
      ...
      qWarning() << "ZIP command not available, but folder is ready:"
                 << distDir;
...
bool Publisher::publishSwitch(const ProjectData &project,
                              const PublishConfig &config) {
  ...
  // Clean previous
  if (QDir(distDir).exists())
    QDir(distDir).removeRecursively();
  QDir().mkpath(distDir);
  ...
  if (bgdiPath.isEmpty()) {
    emit finished(false, "No se encontró bgdi.elf para Switch.\n"
                         "Asegúrate de haber compilado para Switch y que el "
                         "archivo esté en 'runtime/switch/'.");
    return false;
  }
  ...
  if (!devkitPro.isEmpty()) {
    ...
    if (QFile::exists(toolsBin + "/nacptool"))
      nacptool = toolsBin + "/nacptool";
    if (QFile::exists(toolsBin + "/elf2nro"))
      elf2nro = toolsBin + "/elf2nro";
  }

  // Check local runtime tools overrides (Priority)
  QStringList searchPaths;
  searchPaths << appDir << appDir + "/tools" << appDir + "/runtime/switch"
              << appDir + "/runtime/switch/tools";

  for (const QString &path : searchPaths) {
    if (QFile::exists(path + "/nacptool"))
      nacptool = path + "/nacptool";
    if (QFile::exists(path + "/elf2nro"))
      elf2nro = path + "/elf2nro";
  }
  ...
  nacpArgs << "--create" << project.name << config.switchAuthor << version
           << "control.nacp";

  nacpProc.start(nacptool, nacpArgs);
  if (!nacpProc.waitForFinished() || nacpProc.exitCode() != 0) {
    qWarning() << "nacptool failed or not found:"
               << nacpProc.readAllStandardError();
  }
  ...
  if (QFile::exists(nacpFile))
    nroArgs << "--nacp=" + nacpFile;
  ...
  if (!nroProc.waitForFinished() || nroProc.exitCode() != 0) {
    ...
    emit finished(false, "Error al generar ejecutable NRO:\n" + err +
                             "\n\nVerifica que 'elf2nro' está en el PATH o en "
                             "devkitPro/tools/bin.\n" +
                             "Verifica que el icono es formato soportado (JPG "
                             "256x256 por lo general).");
    return false;
  }
...
bool Publisher::publishWeb(const ProjectData &project,
                           const PublishConfig &config) {
  ...
  QString appDir = QCoreApplication::applicationDirPath();
  QString homeRuntime = QDir::homePath() + "/.bennugd2/runtime/web";
  QString webRuntime;

  if (QDir(homeRuntime).exists() && QFile::exists(homeRuntime + "/bgdi.wasm")) {
    webRuntime = homeRuntime;
  } else {
  ...
  if (!QFile::exists(webRuntime + "/bgdi.wasm")) {
    emit finished(false, "No se encontró el runtime web (bgdi.wasm, bgdi.js).\n"
                         "Verifica ~/.bennugd2/runtime/web/ o la carpeta "
                         "runtime/web/ de la aplicación.");
    return false;
  }
  ...
  QFileInfo scriptInfo(project.path + "/" + project.mainScript);
  QString dcbName =
      "game.dcb"; // Always use game.dcb for web loader convenience
  QString sourceDcbPath =
      scriptInfo.absolutePath() + "/" + scriptInfo.baseName() + ".dcb";

  // Check if compiled (we are the publisher, assume calling code ensured
  // compilation or we check)
  if (!QFile::exists(sourceDcbPath)) {
    emit finished(false,
                  "El juego no está compilado (.dcb no encontrado).\nCompila "
                  "el proyecto antes de publicar.");
    return false;
  }
  ...
  if (!checkPy.waitForFinished() || checkPy.exitCode() != 0)
    python = "python";
  ...
  if (packagerScript.isEmpty()) {
    emit finished(false,
                  "No se encontró 'file_packager.py'.\nInstala EMSDK desde el "
                  "diálogo o pon la herramienta en 'runtime/web/tools/'.");
    QDir(dataSrcDir).removeRecursively();
    return false;
  }

  // Arguments: file_packager.py game.data --preload source@/
  // --js-output=game.data.js --no-heap-copy
  QString dataFile = distDir + "/game.data";
  QString jsDataFile = distDir + "/game.data.js";

  QStringList args;
  args << packagerScript << dataFile << "--preload"
       << dataSrcDir + "@/" // Map dataSrcDir root to emscripten root /
       << "--js-output=" + jsDataFile << "--no-heap-copy";
  ...
  QDesktopServices::openUrl(QUrl::fromLocalFile(distDir));
  emit finished(true,
                "Publicación Web completada exitosamente.\nOUTPUT:" + distDir);
  return true;
}

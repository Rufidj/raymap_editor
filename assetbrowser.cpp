...
AssetBrowser::AssetBrowser(QWidget *parent)
    : QWidget(parent), m_watcher(new QFileSystemWatcher(this)) {
  ...
  ""); refreshBtn->setToolTip("Actualizar"); refreshBtn->setMaximumWidth(30);
  ...
  */
  ...
  connect(m_treeWidget, &QTreeWidget::customContextMenuRequested, this,
          &AssetBrowser::onContextMenu);
  connect(m_watcher, &QFileSystemWatcher::directoryChanged, this,
          &AssetBrowser::onDirectoryChanged);
...
void AssetBrowser::addDirectoryItems(QTreeWidgetItem *parent,
                                     const QString &path) {
  QDir dir(path);
  ...
  } else if (dirName == "sound" || dirName == "sounds" || dirName == "audio" ||
             dirName == "music") {
    allowedExtensions << "wav" << "ogg" << "mp3" << "mid" << "mod" << "flac"
                      << "opus";
  } else {
...
QIcon AssetBrowser::getIconForFile(const QString &fileName) {
  ...
  if (ext == "prg") {
    ...
    return QIcon::fromTheme(
        "package-x-generic",
        QApplication::style()->standardIcon(QStyle::SP_DirLinkIcon));
  } else if (ext == "png" || ext == "jpg" || ext == "jpeg" || ext == "bmp") {
    return QIcon::fromTheme(
        "image-x-generic",
        QApplication::style()->standardIcon(QStyle::SP_FileIcon));
  } else if (ext == "wav" || ext == "ogg" || ext == "mp3" || ext == "mid" ||
             ext == "mod" || ext == "flac" || ext == "opus") {
    return QIcon::fromTheme(
        "audio-x-generic",
...
void AssetBrowser::onContextMenu(const QPoint &pos) {
  ...
  qDebug() << "Context menu for:" << path << "isFile:" << info.isFile()
           << "isDir:" << info.isDir();
  ...
  if (info.isFile() && info.suffix().toLower() == "raymap") {
    ...
    QAction *openMapAction =
        menu.addAction(QIcon::fromTheme("document-open"), "Abrir Mapa");
  ...
  if (info.isFile() && info.suffix().toLower() == "fpg") {
    QAction *openFPGAction = menu.addAction(QIcon::fromTheme("image-x-generic"),
                                            "Abrir en Editor FPG");
    connect(openFPGAction, &QAction::triggered, this,
            [this]() { emit fpgEditorRequested(m_selectedPath); });
    menu.addSeparator();
  }

  // Option to delete .scn or .prg files
  if (info.isFile() &&
      (info.suffix().toLower() == "scn" || info.suffix().toLower() == "prg")) {
    QAction *deleteFileAction =
        menu.addAction(QIcon::fromTheme("edit-delete"),
                       info.suffix().toLower() == "scn" ? "Eliminar Escena"
                                                        : "Eliminar Código");
    connect(deleteFileAction, &QAction::triggered, this, [this, path, info]() {
      ...
      QMessageBox::StandardButton reply = QMessageBox::question(
          ...
          QString("¿Estás seguro de que quieres eliminar esta "
      ...
      if (reply == QMessageBox::Yes) {
        if (QFile::remove(path)) {
          QMessageBox::information(
              this, "Éxito", QString("La %1 ha sido eliminada.").arg(fileType));
          refresh(); // Refresh tree
        } else {
          QMessageBox::critical(
              this, "Error",
              QString("No se pudo eliminar la %1.").arg(fileType));
        }
      }
    });
    menu.addSeparator();
  ...
  if (info.isDir()) {
    ...
    if (!path.endsWith("/src")) {
      QAction *renameAction =
          menu.addAction(QIcon::fromTheme("edit-rename"), "Renombrar");
      connect(renameAction, &QAction::triggered, this,
              &AssetBrowser::onRenameFolder);
    }

    QAction *createAction =
        menu.addAction(QIcon::fromTheme("folder-new"), "Nueva Carpeta");
    connect(createAction, &QAction::triggered, this,
            &AssetBrowser::onCreateFolder);

    menu.addSeparator();
  }

  QAction *addFileAction =
      menu.addAction(QIcon::fromTheme("document-new"), "Añadir Archivo...");
  connect(addFileAction, &QAction::triggered, this, &AssetBrowser::onAddFile);

  QAction *newCodeAction = menu.addAction(QIcon::fromTheme("text-x-script"),
                                          "Nuevo Código (.prg)...");
  connect(newCodeAction, &QAction::triggered, this, &AssetBrowser::onNewCode);

  QAction *newSceneAction =
      menu.addAction(QIcon::fromTheme("application-x-executable"),
                     "Nueva Escena 2D (.scn)...");
  ...
  if (info.isDir() && !path.endsWith("/src") && !path.endsWith("/assets") &&
      !path.endsWith("/build")) {
    menu.addSeparator();
    QAction *deleteAction =
        menu.addAction(QIcon::fromTheme("edit-delete"), "Eliminar Carpeta");
    connect(deleteAction, &QAction::triggered, this,
            &AssetBrowser::onDeleteFolder);
  }
...
void AssetBrowser::onRenameFolder() {
  ...
  bool ok;
  QString newName =
      QInputDialog::getText(this, "Renombrar Carpeta",
...
void AssetBrowser::onCreateFolder() {
  ...
  QString folderName = QInputDialog::getText(
      this, "Nueva Carpeta", "Nombre de la carpeta:", QLineEdit::Normal,
      "nueva_carpeta", &ok);
...
void AssetBrowser::onDeleteFolder() {
  ...
  QMessageBox::StandardButton reply = QMessageBox::question(
      this, "Confirmar Eliminación",
      QString("¿Estás seguro de que quieres eliminar la carpeta '%1' y todo su "
              "contenido?\n\n"
              "Esta acción no se puede deshacer.")
          .arg(folderName),
...
void AssetBrowser::onAddFile() {
  ...
  QString filePath = QFileDialog::getOpenFileName(
      this, "Seleccionar Archivo", QDir::homePath(), "Todos los archivos (*)");
  ...
  if (QFile::exists(destPath)) {
    QMessageBox::warning(this, "Error",
                         QString("El archivo '%1' ya existe en esta carpeta.")
  ...
  if (QFile::copy(filePath, destPath)) {
    ...
    QMessageBox::information(this, "Éxito",
                             QString("Archivo '%1' añadido exitosamente")
                                 .arg(sourceFile.fileName()));
  } else {
...
void AssetBrowser::onNewCode() {
  ...
  QString fileName = QInputDialog::getText(
      this, "Nuevo Archivo de Código",
      "Nombre del archivo (sin extensión):", QLineEdit::Normal, "nuevo_proceso",
      &ok);
...
void AssetBrowser::onNewScene() {
  ...
  bool ok;
  QString fileName = QInputDialog::getText(
      this, "Nueva Escena 2D",
      "Nombre de la escena (sin extensión):", QLineEdit::Normal, "nueva_escena",
      &ok);

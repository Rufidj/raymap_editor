...
private slots:
  ...
  void openMapFile(
      const QString &filePath); // Renamed from onOpenMap to avoid ambiguity
  ...
  void onApplyTextureToAllWalls(); // Apply current wall texture to all walls in
                                   // sector
  ...
  // Scene Editor Slots
  void onToggleInteractionPaint(bool enabled);
  void onClearInteractionPaint();
  void onBrushSizeChanged(int size);

  void

  onSetParentSector(); // Assign parent sector to selected sector    // Future
...
private:
  ...
  QAction *m_viewGridAction; // NEW: Replaces m_newAction confusion? Or was it
                             // missing?
  ...
  // Scene Editor UI
  QToolBar *m_sceneToolbar = nullptr;
  QAction *m_paintInteractionAction = nullptr;

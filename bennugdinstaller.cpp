...
BennuGDInstaller::BennuGDInstaller(QWidget *parent)
  ...
  banner->setPixmap(p.scaled(600, 150, Qt::KeepAspectRatioByExpanding,
...
bool BennuGDInstaller::checkMissingRuntimes() {
  ...
  QString root = QDir::homePath() + "/.bennugd2/runtime";
  QStringList platforms = {"linux-gnu", "win64",  "macos",
                           "android",   "switch", "web"};

  for (const QString &p : platforms) {
    if (!QDir(root + "/" + p).exists())
      return true; // Missing
  }
...
void BennuGDInstaller::fetchLatestRelease() {
  ...
  // Linux
  m_downloadQueue.append({"https://github.com/Rufidj/raymap_editor/raw/main/"
                          "runtimes/linux-gnu.tar.gz",
                          "linux-gnu", "linux-gnu.tar.gz"});
  ...
  // MacOS
  m_downloadQueue.append({"https://github.com/Rufidj/raymap_editor/raw/main/"
                          "runtimes/MacOsx.tar.gz",
                          "macos", "macos.tar.gz"});

  // Android
  m_downloadQueue.append({"https://github.com/Rufidj/raymap_editor/raw/main/"
                          "runtimes/android.tar.gz",
                          "android", "android.tar.gz"});

  // Switch
  m_downloadQueue.append({"https://github.com/Rufidj/raymap_editor/raw/main/"
                          "runtimes/switch.tar.gz",
                          "switch", "switch.tar.gz"});
...
void BennuGDInstaller::processDownloadQueue() {
  if (m_downloadQueue.isEmpty()) {
    ...
    QMessageBox::information(this, "Éxito",
                             "Todos los runtimes han sido descargados e "
                             "instalados en ~/.bennugd2/runtime.");
...
void BennuGDInstaller::downloadNextItem() {
  ...
  if (m_currentTask.filename.endsWith(".rar", Qt::CaseInsensitive))
    extension = ".rar";
  else if (m_currentTask.filename.endsWith(".zip", Qt::CaseInsensitive))
    extension = ".zip";
  else if (m_currentTask.filename.endsWith(".tar.gz", Qt::CaseInsensitive))
    extension = ".tar.gz";

  m_tempFilePath =
      QStandardPaths::writableLocation(QStandardPaths::TempLocation) +
      "/bgd2_dl_" + m_currentTask.platform + extension;

  m_currentReply = m_networkManager->get(request);
  connect(m_currentReply, &QNetworkReply::downloadProgress, this,
          &BennuGDInstaller::onDownloadProgress);
  connect(m_currentReply, &QNetworkReply::finished, this,
          &BennuGDInstaller::onDownloadFinished);
}

void BennuGDInstaller::onDownloadProgress(qint64 bytesReceived,
                                          qint64 bytesTotal) {
  if (bytesTotal > 0) {
    int percentage = (int)((bytesReceived * 100) / bytesTotal);
    m_progressBar->setValue(percentage);
    m_statusLabel->setText(QString("Descargando (%1): %2%")
                               .arg(m_currentTask.platform)
                               .arg(percentage));
  }
}
...
void BennuGDInstaller::onDownloadFinished() {
  if (m_currentReply->error() != QNetworkReply::NoError) {
    QMessageBox::warning(this, "Error de Descarga",
                         "Falló descarga de " + m_currentTask.platform + ": " +
...
void BennuGDInstaller::extractAndInstall(const QString &filePath,
  ...
  QString runtimeRoot = QDir::homePath() + "/.bennugd2/runtime";
  ...
  if (filePath.endsWith(".rar", Qt::CaseInsensitive)) {
    ...
    // Assume tar
    program = "tar";
    // Always try to strip one level of folders, as most archives come with a
    // root folder. If this fails (archives without root folder), we might need
    // retry logic or smart detection. But for BennuGD2 official builds, they
    // usually have root folder.
  ...
  if (!finish || extractor.exitCode() != 0) {
    qDebug() << "Extraction failed for " << platform
             << ". STDERR: " << extractor.readAllStandardError();
    ...
    if (platform == "macos") {
      // The files inside MacOsx.tar.gz are seemingly bgdcMac.tgz, bgdiMac.tgz,
      // moddescMac.tgz
      ...
      auto flattenApp = [&](QString appName, QString binaryName) {
        ...
        QStringList potentialNames = {appName, appName + "Mac",
                                      appName + "OSX"};
        QString bundlePath;
        ...
        if (!bundlePath.isEmpty()) {
          ...
          if (QFile::exists(binaryPath)) {
            ...
            QFile(targetPath)
                .setPermissions(QFile::ExeUser | QFile::ExeGroup |
...
// LINUX SPECIAL HANDLING
    ...
    if (platform == "linux") {
      ...
      QDir().mkpath(binDir);
      // Copy binaries
      // TODO: Smarter copy. For now just copy everything from targetDir to
      // binDir
      QProcess cp;
      ...
      QProcess chmod;
      chmod.start("chmod", QStringList()
                               << "+x" << binDir + "/bgdc" << binDir + "/bgdi");

...
public:
  ...
  // Hit testing
  int findSectorAt(
      const QPointF &worldPos); // Made public for use in MainWindow
...
signals:
  ...
  void portalWallSelected(int sectorIndex,
                          int wallIndex); // NEW: For manual portal linking
  void requestDeletePortal(int sectorIndex,
                           int wallIndex); // NEW: Context menu request
  ...
  void requestEditEntityBehavior(
      int index, const EntityInstance &entity); // NEW: Request behavior editor
  void lightSelected(int index,
                     Light light); // NEW: Emitted when a light is selected
...
private:
  ...
  void drawLights(QPainter &painter);   // NEW: Draw omni lights
  ...
  QMap<int, QVector<QPointF>>
      m_originalGroupPositions; // sectorId -> original vertices
  ...
  float pointToLineDistance(const QPointF &point, const QPointF &lineStart,
  ...
  int findVertexAt(const QPointF &worldPos, int &sectorId,
                   float tolerance = 10.0f);
  int findSpawnFlagAt(const QPointF &worldPos, float tolerance = 10.0f);
  int findLightAt(const QPointF &worldPos, float tolerance = 10.0f);

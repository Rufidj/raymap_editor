...
FontEditorDialog::FontEditorDialog(QWidget *parent) : QDialog(parent), currentColor(Qt::white), currentBpp(32), isLoadedFont(false)
{
    setupUi();
...
void FontEditorDialog::setupUi()
{
    ...
    if (idx >= 0) fontFamilyCombo->setCurrentIndex(idx);
    ...
    QVBoxLayout *testLayout = new QVBoxLayout(); // Removed const QObject* parent in constructor? No, QVBoxLayout(QWidget*)
    ...
    textPreviewLabel->setStyleSheet("background-color: #202020; border: 1px solid #404040;");
    ...
    scrollArea->setStyleSheet("QScrollArea { background-color: #202020; border: 1px solid #404040; }");
    ...
    previewLabel->setStyleSheet("background-color: transparent;"); // Transparent so it sits on ScrollArea dark bg
    ...
    // Connections
    connect(previewBtn, &QPushButton::clicked, this, &FontEditorDialog::updatePreview);
    ...
    connect(colorBtn, &QPushButton::clicked, this, &FontEditorDialog::selectColor);
    connect(zoomSlider, &QSlider::valueChanged, this, &FontEditorDialog::updateZoom);
    
    // Auto-update connections
    connect(fontFamilyCombo, SIGNAL(currentIndexChanged(int)), this, SLOT(updatePreview()));
    connect(fontSizeSpin, SIGNAL(valueChanged(int)), this, SLOT(updatePreview()));
    connect(boldCheck, &QCheckBox::toggled, this, &FontEditorDialog::updatePreview);
    connect(italicCheck, &QCheckBox::toggled, this, &FontEditorDialog::updatePreview);
    connect(antialiasCheck, &QCheckBox::toggled, this, &FontEditorDialog::updatePreview);
    connect(bppCombo, SIGNAL(currentIndexChanged(int)), this, SLOT(updatePreview()));
    
    // Text preview update
    connect(testTextInput, &QLineEdit::textChanged, this, &FontEditorDialog::updateTextPreview);
}
...
void FontEditorDialog::saveFont()
{
    ...
    QString filename = QFileDialog::getSaveFileName(this, "Save Font", "", filter);
    ...
    if (currentExt.compare(ext, Qt::CaseInsensitive) != 0) {
        ...
        if (currentExt.compare(".fnt", Qt::CaseInsensitive) == 0 || currentExt.compare(".fnx", Qt::CaseInsensitive) == 0) {
...
QByteArray FontEditorDialog::createFntData()
{
    ...
    if (currentBpp == 8) {
        ...
        // We will make a gradient palette from index 1 to 255 based on chosen color.
        // This allows AA.
        
        QByteArray palData;
        
        // Entry 0: Black/Transparent
        palData.append((char)0); palData.append((char)0); palData.append((char)0);
        ...
        for (int i=1; i<256; i++) {
            ...
            // Bennu 8bit usually doesn't have per-pixel alpha mixing with background unless using special blit flags.
            // But if we want simple anti-aliasing against black background:
            // Color = BaseColor * (i/255).
    ...
    for (int i=0; i<256; i++) {
        if (!glyphs.contains(i)) {
            // Empty char
             for (int k=0; k<7; k++) writeLE32(tableData, 0);
             continue;
        }
            ...
            for (uint32_t x=0; x<g.width; x++) {
                ...
                if (currentBpp == 8) {
                    ...
                    if (alpha > 10) {
                        ...
                        if (color565 == 0) color565 = 1; // Avoid transparent key
                    }
...
{
    QString filename = QFileDialog::getOpenFileName(this, "Abrir Fuente", "", "BennuGD Font (*.fnt *.fnx)");
    if (filename.isEmpty()) return;
    ...
    if (loadFntData(filename)) {
        ...
        QMessageBox::critical(this, "Error", "No se pudo cargar la fuente. Formato desconocido o archivo dañado.");
    }
}

static uint32_t peekLE32(const char* data, int offset) {
    // Check bounds? Caller must ensure
    return (uint8_t)data[offset] | ((uint8_t)data[offset+1] << 8) | ((uint8_t)data[offset+2] << 16) | ((uint8_t)data[offset+3] << 24);
}

// Returns score: -1 (Invalid), >= 0 (Number of valid chars)
static int checkFntIntegrity(const char* ptr, int offset, int dataSize) {
    if (offset + 4 + (256*28) > dataSize) return -1;
    
    uint32_t charset = peekLE32(ptr, offset);
    if (charset > 100) return -1; // Charset is usually 0.
    ...
    for (int i=0; i<256; i++) {
        ...
        // Sanity checks
        if (w > 512 || h > 512) return -1; // Limit for integrity check
...
bool FontEditorDialog::loadFntData(const QString &filename)
{
    ...
    if (loadedBpp == 8) {
        ...
        if (data.size() < offset + 768 + 576) {
            qDebug() << "FontEditor: File too small for palette. Size:" << data.size() << "Needed:" << (offset + 768 + 576);
            return false;
        }
        
        // Skip entry 0 (transparent) - Bennu logic might vary, but usually palette[0] is transparent.
        ...
        qDebug() << "FontEditor: Testing integrity at NoGamma:" << offsetNoGamma << " vs Gamma:" << offsetGamma;
        
        int scoreNoGamma = checkFntIntegrity(ptr, offsetNoGamma, data.size());
        int scoreGamma = checkFntIntegrity(ptr, offsetGamma, data.size());
        
        qDebug() << "FontEditor: Score NoGamma:" << scoreNoGamma << " Score Gamma:" << scoreGamma;
        
        if (scoreNoGamma == -1 && scoreGamma == -1) {
             qDebug() << "FontEditor: Both formats fail integrity check! Trying fallback (Gamma)...";
             offset += 576; 
    ...
    for (int i=0; i<256; i++) {
        ...
        if ((int)d.fo < 0 || (int)d.fo >= data.size()) continue;
        ...
        for (uint32_t y=0; y<d.h; y++) {
            for (uint32_t x=0; x<d.w; x++) {
                if (pOffset >= data.size()) break;

                if (loadedBpp == 8) {
                    uint8_t idx = (uint8_t)ptr[pOffset++];
                    if (idx < palette.size()) img.setPixel(x, y, palette[idx]);
                } else if (loadedBpp == 16) {
                     uint16_t col = (uint8_t)ptr[pOffset] | ((uint8_t)ptr[pOffset+1] << 8);
                     pOffset += 2;
    ...
    if (idx >= 0) bppCombo->setCurrentIndex(idx);
...
void FontEditorDialog::updateZoom()
{
    ...
    if (sz.isEmpty()) sz = QSize(1,1);
    
    Qt::TransformationMode mode = (scale >= 1.0) ? Qt::FastTransformation : Qt::SmoothTransformation;
    
    previewLabel->setPixmap(QPixmap::fromImage(currentAtlas.scaled(sz, Qt::IgnoreAspectRatio, mode)));
}
...
void FontEditorDialog::renderTextPreview(const QString &text)
{
    ...
    for (QChar c : text) {
        ...
        if (glyphs.contains(code)) {
            ...
            totalW += g.xadvance;
            if ((int)g.height > maxH) maxH = g.height;
        } else {
            totalW += 10; // Space for missing
        }
    }
    
    if (totalW == 0) totalW = 10;
    if (maxH == 0) maxH = 20;
    ...
    // User requested dark bg in setupUi label style, so transparent icon on top is fine.
    // Or we can fill with transparent and let QLabel style show through?

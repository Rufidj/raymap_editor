...
void GridEditor::setTextures(const QMap<int, QPixmap> &textures) {
  ...
  // Sync map data textures so they are available when passing MapData to other
  // widgets
  if (m_mapData) {
  ...
  for (int i = 0; i < m_mapData->sectors.size(); i++) {
    ...
    for (int j = 0; j < sector.vertices.size(); j++) {
      ...
      if ((v1.y() > worldPos.y()) != (v2.y() > worldPos.y())) {
        float x_intersect =
            (v2.x() - v1.x()) * (worldPos.y() - v1.y()) / (v2.y() - v1.y()) +
            v1.x();
...
int GridEditor::findWallAt(const QPointF &worldPos, float tolerance) {
  ...
  // This allows selecting walls in other sectors if we click far from current
  // sector walls
  for (int s = 0; s < m_mapData->sectors.size(); s++) {
    // Skip current sector as we already checked it
    if (s == m_selectedSector)
      continue;
...
int GridEditor::findVertexAt(const QPointF &worldPos, int &sectorId,
                             float tolerance) {
  if (!m_mapData)
    return -1;
...
float GridEditor::pointToLineDistance(const QPointF &point,
...
void GridEditor::drawSectors(QPainter &painter) {
  for (int i = 0; i < m_mapData->sectors.size(); i++) {
    ...
    // Fill sector - only highlight if in sector selection mode (not wall
    // selection)
    QColor fillColor;
...
void GridEditor::drawCamera(QPainter &painter) {
  ...
  QPoint dir = worldToScreen(
      QPointF(m_cameraX + std::cos(rot) * 32, m_cameraY - std::sin(rot) * 32));
...
void GridEditor::drawCursorInfo(QPainter &painter) {
  ...
  QString coordText = QString("Cursor: (%.1f, %.1f)")
                          .arg(m_lastCursorPos.x())
                          .arg(m_lastCursorPos.y());
  ...
  if (event->button() == Qt::LeftButton) {
        ...
        if (sectorIdx >= 0) {
          ...
          if (groupId >= 0) {
            ...
            QMessageBox::information(this, tr("Grupo Seleccionado"),
                                     tr("Grupo seleccionado. Arrastra para "
                                        "mover todos los sectores del grupo.\n"
    ...
    if (m_editMode == MODE_DRAW_SECTOR && m_currentPolygon.size() >= 3) {
      ...
      if (m_mapData) {
        ...
        emit sectorCreated(
            newSector.sector_id); // NEW: Notify that sector was created
      }
...
void GridEditor::mouseMoveEvent(QMouseEvent *event) {
  ...
  if (m_isMovingGroup && (event->buttons() & Qt::LeftButton) &&
      m_movingGroupId >= 0 && m_mapData) {
    ...
    if (group) {
      ...
      for (int sectorId : group->sector_ids) {
        ...
        if (sector && m_originalGroupPositions.contains(sectorId)) {
          const QVector<QPointF> &originalVertices =
              m_originalGroupPositions[sectorId];

          // Update vertices from original positions + total offset
          for (int i = 0;
               i < sector->vertices.size() && i < originalVertices.size();
               i++) {
  ...
  if (m_isDraggingSector && m_selectedSector >= 0 &&
      m_selectedSector < m_mapData->sectors.size()) {
    ...
    for (int w = 0; w < sec.walls.size(); w++) {
      ...
      // For smooth dragging, we keep them, but user might need to re-detect if
      // they misalign. Or we could break them. Let's keep them for now as it's
      // less destructive.
    }
  ...
  // Entity dragging
  if (m_isDraggingEntity && m_selectedEntity >= 0 &&
      m_selectedEntity < m_mapData->entities.size()) {
    float dx = worldPos.x() - m_dragStartPos.x();
    float dy = worldPos.y() - m_dragStartPos.y();
    ...
    // We reuse entitySelected to update panel values, OR create a new signal if
    // needed. But MainWindow::onEntityChanged handles updateEntity (which loops
    // back here). Let's check: MainWindow::onEntityChanged calls
    // this->updateEntity. We want to notify MainWindow that entity changed.

    // Actually we need a signal "entityModified" from here to MainWindow.
    // We have `entitySelected` which updates the panel. Let's use that to
    // update the numbers. But we also want to update the Visual Mode. Ideally
    // we emit `updateEntity` signal? No, `updateEntity` method is incoming.
...
void GridEditor::mouseDoubleClickEvent(QMouseEvent *event) {
  ...
  if (m_editMode == MODE_EDIT_VERTICES && event->button() == Qt::LeftButton) {
    ...
    if (wallIdx >= 0 && m_selectedSector >= 0 &&
      ...
      int insertIdx = (wallIdx + 1) % (sector.vertices.size() +
                                       1); // +1 because we are adding one
      if (wallIdx == sector.vertices.size() - 1)
        insertIdx = sector.vertices.size();
      else
        insertIdx = wallIdx + 1;
      ...
      for (int i = 0; i < sector.vertices.size(); i++) {
        ...
        int sourceWallIdx = i;
        if (i > wallIdx)
          sourceWallIdx = i - 1; // Shift back for walls after split
        ...
        if (sourceWallIdx < oldWalls.size()) {
          ...
          if (sourceWallIdx == wallIdx)
            newWall.portal_id = -1;
          else
            newWall.portal_id = oldWall.portal_id;
        }
      ...
      update();
      QMessageBox::information(this, tr("Split Wall"),
                               tr("Wall split successfully!"));
    }
  }
}
...
void GridEditor::drawEntities(QPainter &painter) {
  ...
  for (const EntityInstance &entity : m_mapData->entities) {
    ...
    // Draw diamond shape
    QPolygon diamond;
    diamond << QPoint(pos.x(), pos.y() - 6) << QPoint(pos.x() + 6, pos.y())
            << QPoint(pos.x(), pos.y() + 6) << QPoint(pos.x() - 6, pos.y());
...
void GridEditor::autoParentSector(int sectorIdx) {
  if (!m_mapData || sectorIdx < 0 || sectorIdx >= m_mapData->sectors.size())
    return;
  ...
  for (int i = 0; i < m_mapData->sectors.size(); i++) {
    ...
    if (!otherPoly.boundingRect().contains(currentRect))
      continue;
    ...
    if (otherPoly.containsPoint(currentRect.center(), Qt::OddEvenFill)) {
      ...
      if (allInside) {
        ...
        float area = otherPoly.boundingRect().width() *
                     otherPoly.boundingRect().height();
  ...
  int newParentId =
      (bestParentIdx >= 0) ? m_mapData->sectors[bestParentIdx].sector_id : -1;
  ...
  if (current.parent_sector_id != newParentId) {
    ...
    if (bestParentIdx >= 0) { // Using index directly as we have it
      if (!m_mapData->sectors[bestParentIdx].child_sector_ids.contains(
              current.sector_id)) {
        m_mapData->sectors[bestParentIdx].child_sector_ids.append(
            current.sector_id);
      }
    }
  }

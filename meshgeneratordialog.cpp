...
MeshGeneratorDialog::MeshGeneratorDialog(QWidget *parent) : QDialog(parent) {
  ...
  connect(m_typeCombo, SIGNAL(currentIndexChanged(int)), this,
          SLOT(onTypeChanged(int)));
  ...
  m_depthSpin =
      new QDoubleSpinBox(this); // Depth is usually Y or Z depending on
                                // orientation, let's say "Length/Depth"
  ...
  connect(m_railingsCheck, &QCheckBox::stateChanged, this,
          &MeshGeneratorDialog::updatePreview);
  ...
  connect(m_archCheck, &QCheckBox::stateChanged, this,
          &MeshGeneratorDialog::updatePreview);
  ...
  connect(m_roofTypeCombo, SIGNAL(currentIndexChanged(int)), this,
          SLOT(updatePreview()));
  ...
  connect(expBrowseBtn, &QPushButton::clicked, this,
          &MeshGeneratorDialog::onBrowseExport);
  ...
  connect(m_widthSpin, QOverload<double>::of(&QDoubleSpinBox::valueChanged),
          this, &MeshGeneratorDialog::updatePreview);
  ...
  connect(m_depthSpin, QOverload<double>::of(&QDoubleSpinBox::valueChanged),
          this, &MeshGeneratorDialog::updatePreview);
  connect(m_segmentsSpin, QOverload<int>::of(&QSpinBox::valueChanged), this,
          &MeshGeneratorDialog::updatePreview);
  connect(m_typeCombo, SIGNAL(currentIndexChanged(int)), this,
          SLOT(updatePreview()));
...
void MeshGeneratorDialog::onBrowseExport() {
  QString path = QFileDialog::getSaveFileName(
      this, tr("Exportar MD3"), "model.md3", tr("Modelos MD3 (*.md3)"));
  if (!path.isEmpty()) {
...
MeshGeneratorDialog::MeshParams MeshGeneratorDialog::getParameters() const {
  ...
  params.texturePath =
      params.texturePaths.isEmpty() ? QString() : params.texturePaths.first();
  ...
  params.roofType =
      (MeshParams::RoofType)m_roofTypeCombo->currentData().toInt();
...
void MeshGeneratorDialog::updatePreview() {
  ...
  if (params.texturePaths.size() > 1) {
    ...
    if (!textures.isEmpty()) {
      ...
      // TODO: Adjust mesh UVs to use atlas regions (for now, just show the
      // atlas)
    } else {
  ...
  m_previewWidget->addSurface(mesh, textureToShow);
}

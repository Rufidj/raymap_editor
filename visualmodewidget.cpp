...
VisualModeWidget::VisualModeWidget(QWidget *parent)
    : QOpenGLWidget(parent), m_renderer(nullptr),
      m_updateTimer(new QTimer(this)), m_cameraX(384.0f), m_cameraY(32.0f),
      m_cameraZ(384.0f), m_cameraYaw(0.0f), m_cameraPitch(0.0f),
      m_moveSpeed(200.0f), m_strafeSpeed(200.0f), m_verticalSpeed(150.0f),
      m_mouseSensitivity(0.002f), m_mouseCaptured(false), m_firstMouse(true) {
...
void VisualModeWidget::resizeGL(int w, int h) { glViewport(0, 0, w, h); }
...
void VisualModeWidget::paintGL() {
  ...
  m_renderer->setCamera(m_cameraX, m_cameraY, m_cameraZ, m_cameraYaw,
                        m_cameraPitch);
...
void VisualModeWidget::setMapData(const MapData &mapData, bool resetCamera) {
  ...
  qDebug() << "Visual Mode: Map data set, camera at" << m_cameraX << m_cameraY
           << m_cameraZ;
}
...
void VisualModeWidget::updateCamera(float deltaTime) {
  ...
  // Right vector should be -90 degrees (Clockwise) relative to forward if Y is
  // Down? Let's use standard -PI/2 for Right vector in a math system where
  // angles increase CCW.
  ...
  if (m_keysPressed.contains(Qt::Key_Q)) {
    ...
    static int debugQ = 0;
    if (debugQ++ % 60 == 0)
      qDebug() << "Q pressed - moving up, Y=" << m_cameraY;
  }
  if (m_keysPressed.contains(Qt::Key_E)) {
    m_cameraY -= m_verticalSpeed * deltaTime;
    static int debugE = 0;
    if (debugE++ % 60 == 0)
      qDebug() << "E pressed - moving down, Y=" << m_cameraY;
  }
...
void VisualModeWidget::mouseMoveEvent(QMouseEvent *event) {
  ...
  // Wrap yaw to [0, 2*PI]
  while (m_cameraYaw < 0)
    m_cameraYaw += 2.0f * M_PI;
  while (m_cameraYaw >= 2.0f * M_PI)
    m_cameraYaw -= 2.0f * M_PI;

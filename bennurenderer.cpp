...
bool BennuFont::load(const QString &path) {
  ...
  if (memcmp(ptr, "fnt", 3) == 0) {
    ...
    offset = 7;
    bpp = (uint8_t)ptr[offset++];
  } else
    return false;

  QVector<QRgb> palette;
  if (bpp == 8) {
    if (data.size() < offset + 768)
      return false;
    for (int i = 0; i < 256; i++) {
  ...
  for (int i = 0; i < 256; i++) {
    ...
    if (isFNX) {
      ...
      if (g.width > 0 && g.height > 0 && pixelOffset < (uint32_t)data.size()) {
        ...
        if (po + (int)(g.width * g.height * bytesPerPixel) > data.size())
          continue;
        ...
        for (uint32_t y = 0; y < g.height; y++) {
          for (uint32_t x = 0; x < g.width; x++) {
            if (bpp == 8) {
              ...
              uint8_t b = (uint8_t)ptr[po++];  // byte 0 = Blue
              uint8_t g1 = (uint8_t)ptr[po++]; // byte 1 = Green
              uint8_t r = (uint8_t)ptr[po++];  // byte 2 = Red
              uint8_t a = (uint8_t)ptr[po++];  // byte 3 = Alpha
      ...
      // Old FNT (16 bytes)
      if (tableStart + i * 16 + 16 > data.size())
        break;
      ...
      if (g.width > 0 && g.height > 0 &&
          pixelOffset + (g.width * g.height) <= (uint32_t)data.size()) {
        g.image = QImage(g.width, g.height, QImage::Format_ARGB32);
        int po = pixelOffset;
        for (uint32_t y = 0; y < g.height; y++) {
          for (uint32_t x = 0; x < g.width; x++) {
...
QPixmap BennuFont::render(const QString &text) {
  ...
  for (QChar c : text) {
    int id = c.unicode() % 256;
    if (glyphs.contains(id)) {
      p.drawImage(cx + glyphs[id].xoffset, glyphs[id].yoffset,
                  glyphs[id].image);
...
BennuFontManager &BennuFontManager::instance() {
  static BennuFontManager inst;
  return inst;
}
BennuFontManager::BennuFontManager() {}
QPixmap BennuFontManager::renderText(const QString &text,
...
void BennuFontManager::clearCache() {
  qDeleteAll(m_cache);
  m_cache.clear();
}

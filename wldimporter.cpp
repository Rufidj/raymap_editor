...
void WLDImporter::convertRegionsToSectors(const QVector<WLD_Point> &points,
    ...
    for (int r = 0; r < regions.size(); r++) {
        ...
        // Initialize nested sector fields (WLD doesn't have nesting) - REMOVED for v9 format
    ...
    // BUILD_ENGINE:     // Calculate nested sectors based on portal relationships and wall counts
    // BUILD_ENGINE:     // The region with MORE walls is the parent, the one with FEWER walls is the nested child
    // BUILD_ENGINE:     QMap<QPair<int, int>, bool> processedPairs;
    // BUILD_ENGINE:     
    // BUILD_ENGINE:     for (const Portal &portal : mapData.portals) {
    // BUILD_ENGINE:         int sectorA = portal.sector_a;
    // BUILD_ENGINE:         int sectorB = portal.sector_b;
    // BUILD_ENGINE:         
    // BUILD_ENGINE:         // Avoid processing the same pair twice
    // BUILD_ENGINE:         QPair<int, int> pair1(sectorA, sectorB);
    // BUILD_ENGINE:         QPair<int, int> pair2(sectorB, sectorA);
    // BUILD_ENGINE:         if (processedPairs.contains(pair1) || processedPairs.contains(pair2)) {
    // BUILD_ENGINE:             continue;
    // BUILD_ENGINE:         }
    // BUILD_ENGINE:         processedPairs[pair1] = true;
    // BUILD_ENGINE:         
    // BUILD_ENGINE:         // Find both sectors
    // BUILD_ENGINE:         Sector *secA = nullptr;
    // BUILD_ENGINE:         Sector *secB = nullptr;
    // BUILD_ENGINE:         for (Sector &s : mapData.sectors) {
    // BUILD_ENGINE:             if (s.sector_id == sectorA) secA = &s;
    // BUILD_ENGINE:             if (s.sector_id == sectorB) secB = &s;
    // BUILD_ENGINE:         }
    // BUILD_ENGINE:         
    // BUILD_ENGINE:         if (!secA || !secB) continue;
    // BUILD_ENGINE:         
    // BUILD_ENGINE:         // Determine parent/child based on wall count
    // BUILD_ENGINE:         int wallsA = secA->walls.size();
    // BUILD_ENGINE:         int wallsB = secB->walls.size();
    // BUILD_ENGINE:         
    // BUILD_ENGINE:         Sector *parent = nullptr;
    // BUILD_ENGINE:         Sector *child = nullptr;
    // BUILD_ENGINE:         
    // BUILD_ENGINE:         if (wallsA > wallsB) {
    // BUILD_ENGINE:             parent = secA;
    // BUILD_ENGINE:             child = secB;
    // BUILD_ENGINE:         } else if (wallsB > wallsA) {
    // BUILD_ENGINE:             parent = secB;
    // BUILD_ENGINE:             child = secA;
    // BUILD_ENGINE:         } else {
    // BUILD_ENGINE:             // Same number of walls, use sectorB as parent (matches WLD logic)
    // BUILD_ENGINE:             parent = secB;
    // BUILD_ENGINE:             child = secA;
    // BUILD_ENGINE:         }
    // BUILD_ENGINE:         
    // BUILD_ENGINE:         // Set nested relationship
    // BUILD_ENGINE:         child->parent_sector_id = parent->sector_id;
    // BUILD_ENGINE:         child->sector_type = Sector::NESTED_ROOM;
    // BUILD_ENGINE:         // Parent remains as ROOT (already set during initialization)
    // BUILD_ENGINE:     }
    // BUILD_ENGINE:     
    // BUILD_ENGINE:     qDebug() << "WLDImporter: Calculated nested sector relationships";
...
void WLDImporter::detectPortals(const QVector<WLD_Point> &points,
                                const QVector<WLD_Wall> &walls,
                                MapData &mapData) {
    qDebug() << "WLDImporter: Detecting portals (Build Engine style - using nextsector)";
    
    // BUILD_ENGINE: Instead of creating Portal structures, we directly assign
    // wall.portal_id = connected_sector_id (acts as nextsector)
    ...
    for (Sector &sector : mapData.sectors) {
        for (Wall &sectorWall : sector.walls) {
            ...
            for (int w = 0; w < walls.size(); w++) {
                ...
                if (matches) {
                    // BUILD_ENGINE: Check if this is a portal wall
                    if (wldWall.front_region >= 0 && wldWall.back_region >= 0 &&
                        wldWall.front_region != wldWall.back_region) {
                        
                        // Determine which sector is on the other side
                        int connected_sector = (wldWall.front_region == sector.sector_id) 
                                             ? wldWall.back_region 
                                             : wldWall.front_region;
                        
                        // BUILD_ENGINE: Assign nextsector directly
                        // portal_id now acts as nextsector (connected sector ID)
                        sectorWall.portal_id = connected_sector;
    ...
    qDebug() << "WLDImporter: Assigned" << portals_assigned << "portal connections (nextsector style)";
    
    // Fix textures for portal walls (match original WLD behavior)
    for (Sector &sector : mapData.sectors) {
        for (Wall &wall : sector.walls) {
            if (wall.portal_id >= 0) {

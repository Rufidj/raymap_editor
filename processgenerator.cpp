                                              ...
                                              const QString &wrapperClose)
{
    ...
    if (type == "model") {
        ...
        out << "    model_id = RAY_LOAD_MD3(" << wrapperOpen << "\"" << cleanPath << "\"" << wrapperClose << ");\n";
        out << "    texture_id = map_load(" << wrapperOpen << "\"" << texturePath << "\"" << wrapperClose << ");\n";
        out << "    \n";
        out << "    if (texture_id == 0)\n";
    ...
    } else if (type == "campath" || assetPath.endsWith(".campath", Qt::CaseInsensitive)) {
...
QString ProcessGenerator::generateAllProcessesCode(const QVector<EntityInstance> &entities, 
                                                   const QString &wrapperOpen, 
                                                   const QString &wrapperClose)
{
    QString code;
...
QString ProcessGenerator::generateDeclarationsSection(const QVector<EntityInstance> &entities)
{
...
QString ProcessGenerator::generateProcessCodeWithBehavior(const EntityInstance &entity,
                                                          const QString &wrapperOpen,
                                                          const QString &wrapperClose)
{
    QString code;
    ...
    if (entity.type == "model") {
        ...
        out << "    \n";
        out << "    if (model_id == 0)\n";
        out << "        say(\"[" << entity.processName << "] ERROR: Failed to load model\");\n";
        out << "        RAY_CLEAR_FLAG();\n";
        out << "        return;\n";
        out << "    end\n";
        out << "    \n";
        ...
        out << "    sprite_id = RAY_ADD_SPRITE(world_x, world_y, world_z, 0, 0, 0);\n";
        out << "    if (sprite_id < 0)\n";
        out << "        say(\"[" << entity.processName << "] ERROR: Failed to create sprite\");\n";
        out << "        RAY_CLEAR_FLAG();\n";
        out << "        return;\n";
        out << "    end\n";
        out << "    \n";
        ...
        out << "    campath_id = RAY_LOAD_CAMPATH(" << wrapperOpen << "\"" << cleanPath << "\"" << wrapperClose << ");\n";
        if (wrapperOpen != "") {
             // If we have wrappers, maybe we need extra check or something, 
             // but for now simple check is fine.
        }
        out << "    if (campath_id < 0)\n";
        out << "        say(\"[" << entity.processName << "] ERROR: Failed to load campath\");\n";
        out << "        RAY_CLEAR_FLAG();\n";
        out << "        return;\n";
        out << "    end\n";
        out << "    \n";
    ...
    switch (entity.activationType) {
        case EntityInstance::ACTIVATION_ON_START:
            ...
            if (entity.isPlayer) {
                ...
                out << "        float cam_angle_off = " << (entity.cameraRotation * 0.0174f) << ";\n";
                ...
                switch (entity.controlType) {
                    ...
                    case EntityInstance::CONTROL_THIRD_PERSON:
                        ...
                        out << "        if (key(_w)) dx += cos(angle_milis) * move_speed; dy += sin(angle_milis) * move_speed; end\n";
                        out << "        if (key(_s)) dx -= cos(angle_milis) * move_speed; dy -= sin(angle_milis) * move_speed; end\n";
                        out << "        if (key(_a)) dx += cos(angle_milis + 90000) * move_speed; dy += sin(angle_milis + 90000) * move_speed; end\n";
                        out << "        if (key(_d)) dx += cos(angle_milis - 90000) * move_speed; dy += sin(angle_milis - 90000) * move_speed; end\n";
                        
                        out << "        // Apply movement with collision (Sliding) using object Z\n";
                        out << "        if (RAY_CHECK_COLLISION_Z(world_x, world_y, world_z, world_x + dx, world_y) == 0) world_x += dx; end\n";
                        out << "        if (RAY_CHECK_COLLISION_Z(world_x, world_y, world_z, world_x, world_y + dy) == 0) world_y += dy; end\n";
                        
                        out << "        if (key(_up)) player_pitch += pitch_speed; if (player_pitch > 1.2) player_pitch = 1.2; end end\n";
                        out << "        if (key(_down)) player_pitch -= pitch_speed; if (player_pitch < -1.2) player_pitch = -1.2; end end\n";
                        break;
                    ...
                    case EntityInstance::CONTROL_CAR:
                        ...
                        // Inverted controls based on user feedback (A = Clockwise?, D = Counter-Clockwise?)
                        out << "        if (key(_left) || key(_a)) player_angle -= rot_speed; turn_offset -= 15.0; end\n";
                        out << "        if (key(_right) || key(_d)) player_angle += rot_speed; turn_offset += 15.0; end\n";
                        
                        out << "        if (key(_w) || key(_up)) dx += cos(angle_milis_car) * move_speed; dy += sin(angle_milis_car) * move_speed; end\n";
                        out << "        if (key(_s) || key(_down)) dx -= cos(angle_milis_car) * move_speed; dy -= sin(angle_milis_car) * move_speed; end\n";
                ...
                out << "        RAY_UPDATE_SPRITE_POSITION(sprite_id, world_x, world_y, world_z);\n";
                out << "        RAY_SET_SPRITE_ANGLE(sprite_id, (player_angle * 180.0 / 3.14159) + turn_offset);\n";
                
                if (entity.cameraFollow) {
                    if (entity.controlType == EntityInstance::CONTROL_THIRD_PERSON || entity.controlType == EntityInstance::CONTROL_CAR) {
                        float ox = (entity.cameraOffset_x == 0) ? -200.0f : entity.cameraOffset_x;
                        float oy = entity.cameraOffset_y;
                        float oz = (entity.cameraOffset_z == 0) ? 120.0f : entity.cameraOffset_z;
                        
                        out << "        // Chase Camera - Follows vehicle rotation\n";
                        out << "        float angle_deg = (player_angle + cam_angle_off) * 180000.0 / 3.14159;\n";
                        out << "        s = sin(angle_deg); c = cos(angle_deg);\n";
                        out << "        cx = world_x + c*(" << ox << ") - s*(" << oy << ");\n";
                        out << "        cy = world_y + s*(" << ox << ") + c*(" << oy << ");\n";
                        out << "        \n";
                        out << "        RAY_SET_CAMERA(cx, cy, world_z + (" << oz << "), player_angle + cam_angle_off, player_pitch);\n";
                    }
                    // NOTA: Para First Person no llamamos a RAY_SET_CAMERA porque ya lo gestiona el motor internamente
                }
            }

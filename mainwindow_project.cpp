...
void MainWindow::onNewProject() {
  ...
  if (dialog.exec() == QDialog::Accepted) {
    ...
    if (m_projectManager->createProject(path, name)) {
      ...
      QMessageBox::warning(this, "Error", "No se pudo crear el proyecto.");
    }
  }
}
...
void MainWindow::onOpenProject() {
  ...
  QString dirPath = QFileDialog::getExistingDirectory(
      this, "Seleccionar Carpeta del Proyecto BennuGD2", "",
  ...
  QStringList projFiles =
      dir.entryList(QStringList() << "*.bgd2proj", QDir::Files);
  ...
  if (projFiles.isEmpty()) {
    ...
    QString selected = QInputDialog::getItem(
        this, "Seleccionar Proyecto",
        "Se encontraron múltiples proyectos. Selecciona uno:", projFiles, 0,
        false, &ok);
...
void MainWindow::onCloseProject() {
  ...
  QMessageBox::StandardButton reply =
      QMessageBox::question(this, "Cerrar Proyecto",
...
void MainWindow::regenerateEntityScripts(const ProjectData *customData) {
  ...
  if (customData) {
    ...
    if (data.path.isEmpty())
      data.path = m_projectManager->getProjectPath();
  } else {
    data =
        m_projectManager->loadProjectData(m_projectManager->getProjectPath());
  }
  ...
  for (const EntityInstance &ent : mapData->entities) {
    if (ent.type == "model" && !ent.assetPath.isEmpty()) {
      ...
      if (generatedFiles.contains(hFileName))
        continue;
      ...
      QString code = ProcessGenerator::generateProcessCode(
          ent.processName, relativeAssetPath, ent.type,
          generator.getWrapperOpen(), generator.getWrapperClose());
...
void MainWindow::openProject(const QString &path) {
  ...
  // Clear any existing tabs (including default empty map created by
  // onCloseProject or initial state)
  while (m_tabWidget->count() > 0) {
    QWidget *w = m_tabWidget->widget(0);
    m_tabWidget->removeTab(0);
    // Ensure widget is deleted if it's not parented anymore or just safe
    // cleanup
    delete w;
  }
  ...
  if (m_projectManager->openProject(path)) {
    ...
    // Auto-open Scenes first (.scn) - New Workflow
    QDir projectDir(m_projectManager->getProjectPath());
    QStringList scnFilters;
    scnFilters << "*.scn";
    QFileInfoList sceneFiles =
        projectDir.entryInfoList(scnFilters, QDir::Files);

    // Also check assets/scenes (Common location)
    QDir scenesSubDir(m_projectManager->getProjectPath() + "/assets/scenes");
    if (scenesSubDir.exists()) {
      sceneFiles.append(scenesSubDir.entryInfoList(scnFilters, QDir::Files));
    }

    // Also check assets root
    QDir assetsDir(m_projectManager->getProjectPath() + "/assets");
    if (assetsDir.exists()) {
      sceneFiles.append(assetsDir.entryInfoList(scnFilters, QDir::Files));
    }

    bool scenesOpened = false;
    for (const QFileInfo &sceneFile : sceneFiles) {
      qDebug() << "Auto-opening scene:" << sceneFile.absoluteFilePath();
      onOpenScene(sceneFile.absoluteFilePath());
      scenesOpened = true;
    }

    if (scenesOpened) {
      m_statusLabel->setText(
          tr("Proyecto cargado: %1 escenas abiertas").arg(sceneFiles.size()));
      // Select the first tab (usually the main scene if sorted alphabetically
      // or the first found)
      if (m_tabWidget->count() > 0) {
        m_tabWidget->setCurrentIndex(0);
      }
    } else {
      // Fallback: Auto-open maps from assets/maps (Legacy Workflow)
      ...
      if (dir.exists()) {
        ...
        if (!mapFiles.isEmpty()) {
          m_statusLabel->setText(
              tr("Proyecto cargado: %1 mapas abiertos").arg(mapFiles.size()));
        }
      }
    }
  } else {
...
void MainWindow::onStartupSceneRequested(const QString &sceneName) {
  ...
  if (generator.setStartupScene(m_projectManager->getProjectPath(),
                                sceneName)) {
    // Ensure everything is synchronized (includes, resources, etc.)
    regenerateEntityScripts(&data);

    QMessageBox::information(this, "Escena Inicial",
                             "Se ha configurado '" + sceneName +
                                 "' como escena inicial en el proyecto.");
  } else {

...
EntityPropertyPanel::EntityPropertyPanel(QWidget *parent)
    : QWidget(parent), m_currentIndex(-1), m_updating(false) {
  QVBoxLayout *mainLayout = new QVBoxLayout(this);
  mainLayout->setContentsMargins(0, 0, 0, 0);

  m_editBehaviorButton = new QPushButton(tr("Editar Comportamiento..."));
  m_editBehaviorButton->setIcon(QIcon::fromTheme("system-run")); // or similar
  connect(m_editBehaviorButton, &QPushButton::clicked, this, [this]() {
    if (m_currentIndex >= 0) {
      emit editBehaviorRequested(m_currentIndex, m_currentEntity);
    }
  });
  mainLayout->addWidget(m_editBehaviorButton);

  QGroupBox *groupBox =
      new QGroupBox(); // Removed title to avoid duplication with Dock title
  QFormLayout *layout = new QFormLayout(groupBox);

  // Type / Name
  m_nameEdit = new QLineEdit();
  connect(m_nameEdit, &QLineEdit::editingFinished, this,
          &EntityPropertyPanel::onTypeChanged);
...
void EntityPropertyPanel::setupSpinBox(QDoubleSpinBox *spin,
  ...
  connect(spin, QOverload<double>::of(&QDoubleSpinBox::valueChanged), this,
          &EntityPropertyPanel::onValueChanged);
}
...
void EntityPropertyPanel::onTypeChanged() {
  ...
  // Type and Asset Path usually shouldn't be changed manually here without
  // updating the asset, but processName is fine.

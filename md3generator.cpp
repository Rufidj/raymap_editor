...
bool MD3Generator::generateAndSave(MeshType type, float width, float height,
  ...
  if (success && !texturePath.isEmpty() && QFile::exists(texturePath)) {
    ...
    QString destTexturePath =
        mdlInfo.absolutePath() + "/" + mdlInfo.baseName() + "." + textureExt;
...
MD3Generator::MeshData MD3Generator::generateMesh(MeshType type, float width,
                                                  float height, float depth,
                                                  int segments) {
  MeshData mesh;
  switch (type) {
  case RAMP:
    mesh = generateRamp(width, height, depth);
    break;
  case STAIRS:
    mesh = generateStairs(width, height, depth, segments);
    break;
  case BOX:
    mesh = generateBox(width, height, depth);
    break;
  case CYLINDER:
    mesh = generateCylinder(width, height, depth, segments);
    break;
  case BRIDGE:
    mesh = generateBridge(width, height, depth, true, false);
    break; // Railings yes, arch no by default
  case HOUSE:
    mesh = generateHouse(width, height, depth, 2);
    break; // Gabled roof by default
  case ARCH:
    mesh = generateArch(width, height, depth, qMax(segments, 8));
    break;
  }
  return mesh;
}
...
MD3Generator::MeshData MD3Generator::generateMesh(MeshType type, float width,
  ...
  return generateMesh(type, width, height, depth, segments, hasRailings, false,
                      roofType);
}
...
MD3Generator::MeshData MD3Generator::generateMesh(MeshType type, float width,
  ...
  case CYLINDER:
    ...
    break;
  case BRIDGE:
    mesh = generateBridge(width, height, depth, hasRailings, hasArch);
    break;
  case HOUSE:
    mesh = generateHouse(width, height, depth, roofType);
    break;
  case ARCH:
    mesh = generateArch(width, height, depth, qMax(segments, 8));
    break;
...
MD3Generator::MeshData MD3Generator::generateRamp(float width, float height,
                                                  float depth) {
  MeshData mesh;
  float w2 = width / 2.0f;
  ...
  // Vertices needed for sharp edges -> split normals -> unique vertices per
  // face. Faces: Bottom, Back, Left, Right, Sloped Top. 5 Faces. Bottom: 2 Tris
  // Back: 2 Tris
  ...
  QVector3D slopeN =
      QVector3D::crossProduct(v_br - v_bl, v_tl - v_bl).normalized();
  ...
  // Actually, let's just make the Ramp face for now as that's the most visible
  // part. But we need a closed mesh for shadows etc.
...
MD3Generator::MeshData MD3Generator::generateBox(float width, float height,
                                                 float depth) {
  MeshData mesh;
  float w2 = width / 2.0f;
  float h = height;
  ...
  auto addFace = [&](QVector3D v1, QVector3D v2, QVector3D v3, QVector3D v4,
                     QVector3D normal) {
...
MD3Generator::MeshData MD3Generator::generateStairs(float width, float height,
  ...
  // Helper lambda to add face (copied from box, duplicate code acceptable for
  // now or move to helper)
  auto addFace = [&](QVector3D v1, QVector3D v2, QVector3D v3, QVector3D v4,
                     QVector3D normal, float uvYStart, float uvYEnd,
                     float uvZStart = 0, float uvZEnd = 0) {
    int base = mesh.vertices.size();
  ...
  // Generate each step as a box-like structure (but optimized? No, just draw
  // visible faces) Actually, simple way: Just draw blocks.
  for (int i = 0; i < steps; i++) {
    float yFront = i * stepDepth;
    float yBack = (i + 1) * stepDepth;
    float zBottom =
        0; // Usually solid stairs go to ground? Or floating? Solid is better.
  ...
  mesh.vertices.append(
      {QVector3D(-w2, 0, 0), QVector3D(0, 0, -1), QVector2D(0, 0)});
  mesh.vertices.append(
      {QVector3D(w2, 0, 0), QVector3D(0, 0, -1), QVector2D(1, 0)});
  mesh.vertices.append(
      {QVector3D(w2, depth, 0), QVector3D(0, 0, -1), QVector2D(1, 1)});
  mesh.vertices.append(
      {QVector3D(-w2, depth, 0), QVector3D(0, 0, -1), QVector2D(0, 1)});
...
MD3Generator::MeshData MD3Generator::generateCylinder(float width, float height,
                                                      float depth,
                                                      int segments) {
  MeshData mesh;
  if (segments < 3)
  ...
  float radiusY =
      depth /
      2.0f; // Elliptical cylinder support? Assume circle for now, or scaled.
  ...
  for (int i = 0; i < segments; i++) {
    ...
    QVector2D uv1((topVerts[i].x() / width) + 0.5,
                  (topVerts[i].y() / depth) + 0.5);
    QVector2D uv2((topVerts[i + 1].x() / width) + 0.5,
                  (topVerts[i + 1].y() / depth) + 0.5);
...
MD3Generator::MeshData MD3Generator::generateBridge(float width, float height,
                                                    float depth,
                                                    bool hasRailings,
                                                    bool hasArch) {
  MeshData mesh;
  ...
  // textureIndex: 0 = surface (left half of atlas), 1 = walls/railings (right
  // half)
  auto addFace = [&](QVector3D v1, QVector3D v2, QVector3D v3, QVector3D v4,
                     QVector3D normal, int textureIndex = 0) {
    // Calculate UV offset based on texture index (for 2-texture horizontal
    // atlas)
  ...
  if (!hasArch) {
    ...
    if (archHalfWidth < w2) {
      ...
      addFace(QVector3D(-w2, 0, archRadius), QVector3D(-w2, depth, archRadius),
              QVector3D(-w2, depth, h), QVector3D(-w2, 0, h),
              QVector3D(-1, 0, 0), 1);

      // Left inner wall (X = -archHalfWidth) - connects to arch
      addFace(QVector3D(-archHalfWidth, 0, h),
              QVector3D(-archHalfWidth, depth, h),
              QVector3D(-archHalfWidth, depth, archRadius),
              QVector3D(-archHalfWidth, 0, archRadius), QVector3D(1, 0, 0), 1);

      // Left wall top (connects outer to inner)
      addFace(QVector3D(-w2, 0, h), QVector3D(-w2, depth, h),
              QVector3D(-archHalfWidth, depth, h),
              QVector3D(-archHalfWidth, 0, h), QVector3D(0, 0, 1), 0);

      // Left wall BOTTOM (fills gap between bridge edge and arch)
      addFace(QVector3D(-archHalfWidth, 0, 0),
              QVector3D(-archHalfWidth, depth, 0), QVector3D(-w2, depth, 0),
              QVector3D(-w2, 0, 0), QVector3D(0, 0, -1), 1);

      // Left wall FRONT face (Y=0, connects arch to wall)
      addFace(QVector3D(-w2, 0, 0), QVector3D(-archHalfWidth, 0, 0),
              QVector3D(-archHalfWidth, 0, archRadius),
              QVector3D(-w2, 0, archRadius), QVector3D(0, -1, 0), 1);

      // Left wall BACK face (Y=depth, connects arch to wall)
      addFace(QVector3D(-archHalfWidth, depth, 0), QVector3D(-w2, depth, 0),
              QVector3D(-w2, depth, archRadius),
              QVector3D(-archHalfWidth, depth, archRadius), QVector3D(0, 1, 0),
              1);

      // Right outer wall (X = w2)
      addFace(QVector3D(w2, 0, h), QVector3D(w2, depth, h),
              QVector3D(w2, depth, archRadius), QVector3D(w2, 0, archRadius),
              QVector3D(1, 0, 0), 1);

      // Right inner wall (X = archHalfWidth) - connects to arch
      addFace(QVector3D(archHalfWidth, 0, archRadius),
              QVector3D(archHalfWidth, depth, archRadius),
              QVector3D(archHalfWidth, depth, h),
              QVector3D(archHalfWidth, 0, h), QVector3D(-1, 0, 0), 1);

      // Right wall top (connects outer to inner)
      addFace(QVector3D(archHalfWidth, 0, h),
              QVector3D(archHalfWidth, depth, h), QVector3D(w2, depth, h),
              QVector3D(w2, 0, h), QVector3D(0, 0, 1), 0);

      // Right wall BOTTOM (fills gap between bridge edge and arch)
      addFace(QVector3D(w2, 0, 0), QVector3D(w2, depth, 0),
              QVector3D(archHalfWidth, depth, 0),
              QVector3D(archHalfWidth, 0, 0), QVector3D(0, 0, -1), 1);

      // Right wall FRONT face (Y=0, connects arch to wall)
      addFace(QVector3D(archHalfWidth, 0, 0), QVector3D(w2, 0, 0),
              QVector3D(w2, 0, archRadius),
              QVector3D(archHalfWidth, 0, archRadius), QVector3D(0, -1, 0), 1);

      // Right wall BACK face (Y=depth, connects arch to wall)
      addFace(QVector3D(w2, depth, 0), QVector3D(archHalfWidth, depth, 0),
              QVector3D(archHalfWidth, depth, archRadius),
              QVector3D(w2, depth, archRadius), QVector3D(0, 1, 0), 1);
    }

    // Front and back walls (full height)
    addFace(QVector3D(-w2, 0, 0), QVector3D(w2, 0, 0), QVector3D(w2, 0, h),
            QVector3D(-w2, 0, h), QVector3D(0, -1, 0), 1);

    addFace(QVector3D(w2, depth, 0), QVector3D(-w2, depth, 0),
            QVector3D(-w2, depth, h), QVector3D(w2, depth, h),
            QVector3D(0, 1, 0), 1);
  }
...
MD3Generator::MeshData MD3Generator::generateHouse(float width, float height,
                                                   float depth, int roofType) {
  MeshData mesh;
  float w2 = width / 2.0f;
  ...
  if (roofType == 0) { // FLAT
    ...
    // Right slope
    QVector3D rn =
        QVector3D::crossProduct(w6 - peak1, peak2 - peak1).normalized();
...
MD3Generator::MeshData MD3Generator::generateArch(float width, float height,
                                                  float depth, int segments) {
  MeshData mesh;
  if (segments < 4)
    segments = 4;
  ...
  addFace(QVector3D(-w2, 0, height), QVector3D(-w2 + pillarWidth, 0, height),
          QVector3D(-w2 + pillarWidth, 0, 0), QVector3D(-w2, 0, 0),
          QVector3D(0, -1, 0));
  addFace(QVector3D(-w2, archThickness, 0),
          QVector3D(-w2 + pillarWidth, archThickness, 0),
          QVector3D(-w2 + pillarWidth, archThickness, height),
          QVector3D(-w2, archThickness, height), QVector3D(0, 1, 0));

  // Right pillar
  addFace(QVector3D(w2 - pillarWidth, 0, height), QVector3D(w2, 0, height),
          QVector3D(w2, 0, 0), QVector3D(w2 - pillarWidth, 0, 0),
          QVector3D(0, -1, 0));
  addFace(QVector3D(w2 - pillarWidth, archThickness, 0),
          QVector3D(w2, archThickness, 0), QVector3D(w2, archThickness, height),
          QVector3D(w2 - pillarWidth, archThickness, height),
          QVector3D(0, 1, 0));
  ...
  for (int i = 0; i < segments; i++) {
    ...
    addFace(outerCurve[i], outerCurve[i + 1],
            ...
            QVector3D(outerCurve[i].x(), archThickness, height),
            QVector3D(0, 1, 0));
  }
...
bool MD3Generator::saveMD3(const MeshData &mesh, const QString &filename) {
  ...
  int num_frames =
      mesh.animationFrames.isEmpty() ? 1 : mesh.animationFrames.size();
  int num_surfaces = 1;
  int num_tags = 0;
  ...
  header.ofs_tags = ofs_tags;     // 0 tags -> same offset? No, typically offset
                                  // points to start of tags.
  ...
  int num_verts = mesh.animationFrames.isEmpty()
                      ? mesh.vertices.size()
                      : mesh.animationFrames[0].size();
  ...
  surface.ofs_triangles =
      surface.ofs_shaders + num_shaders * sizeof(MD3::Shader);
  ...
  surface.ofs_end =
      surface.ofs_xyznormal + num_verts * sizeof(MD3::Vertex) * num_frames;

  out.writeRawData(reinterpret_cast<const char *>(&surface),
                   sizeof(MD3::Surface));
  ...
  out.writeRawData(reinterpret_cast<const char *>(&shader),
                   sizeof(MD3::Shader));
  ...
  for (int i = 0; i < num_tris; i++) {
    ...
    tri.indexes[1] = mesh.indices[i * 3 + 1];
    tri.indexes[2] = mesh.indices[i * 3 + 2];
    out.writeRawData(reinterpret_cast<const char *>(&tri),
                     sizeof(MD3::Triangle));
  }
  ...
  for (int i = 0; i < num_verts; i++) {
    ...
    if (mesh.animationFrames.isEmpty()) {
      ...
      st.st[1] = 1.0f - mesh.vertices[i].uv.y();
    } else {
      st.st[0] = mesh.animationFrames[0][i].uv.x();
      st.st[1] = 1.0f - mesh.animationFrames[0][i].uv.y();
    }
    out.writeRawData(reinterpret_cast<const char *>(&st),
                     sizeof(MD3::TexCoord));
  }

  // Vertices (XYZ + Normal) - Multi-frame
  for (int f = 0; f < num_frames; f++) {
    for (int i = 0; i < num_verts; i++) {
      MD3::Vertex v;
      const auto &vd = mesh.animationFrames.isEmpty()
                           ? mesh.vertices[i]
                           : mesh.animationFrames[f][i];
      v.coord[0] = (int16_t)(vd.pos.x() * 64.0f);
      v.coord[1] = (int16_t)(vd.pos.y() * 64.0f);
      v.coord[2] = (int16_t)(vd.pos.z() * 64.0f);
      encodeNormal(vd.normal, v.normal);
